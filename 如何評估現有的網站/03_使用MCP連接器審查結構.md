...
> [!TIP]
> 🎯 **推薦實作範例：台灣高鐵首頁的「時刻表與票價」與「網路訂票」控制區**
> 對於沒有經驗的學生，這是評估網頁轉換率最貼近日常且效果顯著的起點。
> * **評估重點**：高鐵首頁最核心的兩個切換卡（Tab）——**「時刻表與票價」**與**「網路訂票」**，是使用者進入高鐵旅程的第一步。學生可以利用 AI 分析這兩個控制區內各欄位（出發站、到達站、日期、時間等下拉選單及輸入框）的代碼結構，找出可能降低查詢轉換率的介面盲點（例如：下拉選單的無障礙支援、預設提示文字等）。
> * **練習網址**：`https://www.thsrc.com.tw/`
> * **操作方式**：
>   1. 在你的 Chrome 瀏覽器中打開台灣高鐵官網。
>   2. 分別點選切換首頁核心的「時刻表與票價」與「網路訂票」分頁卡（Tabs），並讓你的瀏覽器停留在高鐵首頁。
>   3. 在對話框中發送下方 Prompt，AI 將會使用連接器直接讀取你當前高鐵首頁的實時 DOM 代碼。

> **最佳評估方式**：在對話中呼叫 **Control Chrome** 讀取台灣高鐵官網首頁的實時 DOM 樹與欄位結構。

```markdown
# 任務
請使用連接器讀取我當前瀏覽器分頁的高鐵官網首頁 DOM 結構，重點評估首頁核心「時刻表與票價」與「網路訂票」這兩個切換分頁卡（Tabs）及其相關輸入欄位（出發站、到達站、日期、時間等控制項）的易用性與技術阻力。

# 評估項目
1. **控制項親和度**：這些下拉選單與日期選擇控制項的 HTML 標籤結構是否健全？是否容易造成使用者點擊困難？
2. **切換卡體驗**：兩個 Tab（時刻表與票價、網路訂票）的 DOM 結構與 ARIA 屬性（無障礙）是否能讓螢幕朗讀軟體或鍵盤操作順利切換？
3. **資訊預設與提示**：輸入欄位是否有良好的預設值或引導提示文字？
4. **優化代碼建議**：請提供這兩個控制區前端 HTML/CSS 結構的重構方案，說明如何透過優化表單控制項來提升使用者的查詢轉換率。
```
