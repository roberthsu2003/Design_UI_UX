...
> [!TIP]
> **💡 教學指引：如何向學生解釋「方法二（文字與網址）」與「方法三（程式碼結構）」的差別？**
> 
> 雖然方法二與方法三都屬於「非視覺（不看圖）」的評估，但兩者的**評估視角與目的**截然不同：
> * **方法二（純文字與 URL）——「黑箱測試 / 讀者視角」**
...
> 
> * **方法三（HTML / CSS / JS 結構）——「白箱測試 / 工程師視角」**
>   * **操作**：複製網頁的底層原始碼（如表單 DOM 結構、CSS 樣式或前端 JS 驗證邏輯）貼給 AI。
>   * **評估重點**：看**底層骨架與技術規範**。例如：圖片有沒有寫 `alt` 屬性？按鈕是否使用了語意化標籤（影響盲人螢幕閱讀器）？CSS 佈局是否有潛在的跑版漏洞？JS 是否有過濾惡意輸入？
...
#### Q3. 如果我想在 Claude Desktop 中使用這個功能，該如何設定？
**你可以在 Claude Desktop 中載入 Puppeteer MCP，讓它能夠自動開啟本地瀏覽器讀取網頁並截圖：**

1. **確保安裝 Node.js**：開啟終端機（Terminal）輸入 `node -v` 確認是否有安裝 Node.js 環境。若無，請先至 [Node.js 官網](https://nodejs.org/) 下載安裝。
2. **開啟或建立設定檔**：在 Mac 上，開啟或建立以下路徑的 JSON 設定檔：
   `~/Library/Application Support/Claude/claude_desktop_config.json`
3. **寫入 MCP 配置**：將以下 JSON 內容寫入並存檔：
   ```json
   {
     "mcpServers": {
       "puppeteer": {
   ...
   ```
4. **重啟 Claude Desktop**：徹底關閉 Claude Desktop 應用程式（`Cmd + Q`）並重新開啟。若看見對話輸入框出現**小槌子圖示 (🔨)**，即代表載入成功。
5. **開始發問**：你可以直接輸入指令，例如：「*請幫我開啟網頁 https://www.thsrc.com.tw/ ，幫我擷取首頁畫面，並評估其視覺層次與可用性是否需要優化？*」
...
### 🌐 強大內建助手：如何使用 Gemini in Chrome 進行即時網頁評估？

對於經常進行網頁審查的專案管理員，Google Chrome 瀏覽器中已深度整合了 **Gemini**，您不需要頻繁複製代碼或頻繁切換視窗，即可在瀏覽網頁的當下進行即時檢測。

> [!NOTE]
> **主要支援兩大使用場景：**
>
> 1. **Chrome 側邊欄 Gemini (Gemini in Chrome Side Panel)**
>    * **如何啟用**：點擊 Chrome 右上角的「Gemini 圖標」（或透過網址列輸入 `@gemini` 並按下 Space 鍵）。
>    * **評估應用**：您可以直接在側邊欄對著目前瀏覽的網頁發問。例如輸入：「*評估此頁面的資訊架構與視覺層次是否清晰，並給予改進建議。*」
> 
> 2. **Chrome 開發者工具中的 AI 輔助 (AI Assistance in Chrome DevTools)**
>    * **如何啟用**：按 `F12` 開啟開發者工具，在面板列表中尋找 **AI assistance**（或右鍵點擊網頁上的任何元素，選擇 **「詢問 AI / Ask AI」**）。
>    * **評估應用**：
>      * **UI/CSS 審查**：選中某個按鈕或區塊，讓 AI 分析其 CSS 樣式並給予更符合現代設計規範的修改建議。
>      * **無障礙 (A11y) 審查**：選中表單或導覽列，詢問 AI：「*此 HTML 結構符合 WCAG 無障礙規範嗎？*」，AI 會直接分析 DOM 結構並給出優化程式碼。
>      * **效能與除錯**：請 AI 解釋 Console 中的錯誤警告，或優化載入效能。
